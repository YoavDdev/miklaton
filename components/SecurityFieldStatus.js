...
export default function SecurityFieldStatus() {
  ...
  // Only show finished entries for 4 hours after the shift ended
  const FINISHED_WINDOW_MINUTES = 4 * 60;
  const hasEnded = (startTime, endTime, fromYesterday = false) => {
    ...
    if (fromYesterday && endMinutes < startMinutes) {
      return currentMinutes >= endMinutes && (currentMinutes - endMinutes) <= FINISHED_WINDOW_MINUTES;
    }
    ...
    return currentMinutes >= endMinutes && currentMinutes >= startMinutes && (currentMinutes - endMinutes) <= FINISHED_WINDOW_MINUTES;
  };
