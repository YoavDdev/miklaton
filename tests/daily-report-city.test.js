...
describe('כתובת האירוע', () => {
  const at = (location) =>
    mapCityEvents([wpEvent('20260826', '09:00:00', 'אירוע', location)], new Date('2026-08-26T12:00:00'))[0]
      .name;

  it('הכתובת נכללת בשורה', () => {
    expect(at('רחוב רם כהן 5')).toBe('אירוע - רחוב רם כהן 5');
  });

  it('מסיר את שם העיר מסוף הכתובת', () => {
    expect(at('הדגנים 54 יהוד-מונוסון')).toBe('אירוע - הדגנים 54');
  });

  it('לא משאיר פסיק תלוי אחרי הסרת שם העיר', () => {
    expect(at("מייקלאב. רחוב אברהם גירון 3 א', קומה 1, יהוד - מונוסון")).toBe(
      "אירוע - מייקלאב. רחוב אברהם גירון 3 א', קומה 1"
    );
  });

  it('בלי כתובת - רק שם האירוע', () => {
    expect(at('')).toBe('אירוע');
  });

  it('כתובת שהיא רק שם העיר יורדת לגמרי', () => {
    expect(at('יהוד - מונוסון')).toBe('אירוע');
  });
});
