...
/**
 * חלון הכוננות של החג, כתאריכים.
 *
 * למה זה לא זהה לשעות החג: מבחינת המוקד הכוננות מתחילה עם היום ולא עם
 * הדלקת הנרות, וכשחג נופל בסוף שבוע ההסדר מתחיל כבר ביום שישי בבוקר.
 * בנוסף, לכל חג יש התנהגות אחרת שאי אפשר לגזור מלוח השנה - ולכן מנהל
 * המוקד יכול לקבוע את החלון בעצמו. ברירת המחדל היא ימי החג עצמם.
 *
 * שני הקצוות נדרשים יחד; הגדרה חלקית נופלת חזרה לברירת המחדל, כדי שלא
 * ייווצר חלון הפוך או פתוח מצד אחד.
 */
export function dutyWindow(period) {
  if (period?.duty_start_date && period?.duty_end_date) {
    return { start: period.duty_start_date, end: period.duty_end_date };
  }
  return {
    start: israelDate(new Date(period.starts_at)),
    end: israelDate(new Date(period.ends_at)),
  };
}

/** כל ימי חלון הכוננות, כולל שני הקצוות. */
export function dutyDates(period) {
  const { start, end } = dutyWindow(period);
  const out = [];
  let cur = start;
  // תקרה של 60 יום כדי ששגיאת קלט לא תייצר לולאה אינסופית
...
/** האם חלון הכוננות של התקופה חל על עכשיו, לפי התאריך בישראל. */
export function isDutyActive(period, now = new Date()) {
  const { start, end } = dutyWindow(period);
  const today = israelDate(now);
  return today >= start && today <= end;
}

export function findActiveDutyPeriod(periods, now = new Date()) {
  return (periods || []).find((p) => isDutyActive(p, now)) || null;
}

/** מספר הימים השלמים מהיום בישראל עד תאריך נתון. שלילי אם עבר. */
export function daysUntil(isoDate, now = new Date()) {
  const toUtc = (d) => {
    const [y, m, day] = d.split('-').map(Number);
    return Date.UTC(y, m - 1, day);
  };
  return Math.round((toUtc(isoDate) - toUtc(israelDate(now))) / 86400000);
}
