...
/**
 * זיהוי רכב מתוך הערות השיבוץ (YOA-37): הקובץ כותב קיצור ("קשקאי",
 * "סיאט"), רשימת הרכבים בהגדרות מחזיקה שמות מלאים ("ניסאן קשקאי").
 * ההתאמה בזמן קריאה, כך שהיא עובדת גם על שבועות שכבר יובאו. קיצור
 * שמתאים ליותר מרכב אחד לא מנחש.
 */
export function vehicleFromNotes(notes, vehicles) {
  if (!notes || !Array.isArray(vehicles) || vehicles.length === 0) return null;
  for (const raw of notes.split('|')) {
    const token = raw.trim();
    if (!token || /\d/.test(token)) continue;
    const matches = vehicles.filter(v => v.includes(token) || token.includes(v));
