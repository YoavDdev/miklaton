...
import { findActiveDutyPeriod } from '@/lib/holidays';
...
export async function loadPeriods(municipalityId) {
  ...
  const { data, error } = await supabase
    ...
    .select('id, name, slug, starts_at, ends_at, status, notes, duty_start_date, duty_end_date')
...
export async function getActivePeriod(municipalityId, now = new Date()) {
  return findActiveDutyPeriod(await loadPeriods(municipalityId), now);
}
