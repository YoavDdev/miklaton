...
/**
 * טוקן קבוע לצפייה בלוח כוננות החג בלי התחברות.
 *
 * מנהל המוקד והמוקדנים שולחים את הקישור בוואטסאפ לגורמים שאין להם חשבון
 * במערכת. הדף שנפתח הוא לקריאה בלבד.
 *
 * מרחב חתימה נפרד מ-signDutyFormToken ומ-signScheduleUploadToken למרות אותו
 * סוד: קישור לוח חג שדלף אינו מקנה גישה לטופס תורנות או להעלאת סידור, ולהפך.
 *
 * שים לב: הטוקן קבוע ואינו פג, כמו שאר הקישורים הנשלחים בוואטסאפ. מי שמעביר
 * את הקישור הלאה מעביר גישה לשמות ולטלפונים של הכוננים.
 */
export function signHolidayBoardToken(periodId) {
  const secret = process.env.DUTY_FORM_SECRET;
  if (!secret) {
    throw new Error('DUTY_FORM_SECRET environment variable is not set');
  }
  return crypto
    .createHmac('sha256', secret)
    .update(`holiday-board:${periodId}`)
    .digest('hex')
    .slice(0, 32);
}

export function verifyHolidayBoardToken(periodId, token) {
  if (!process.env.DUTY_FORM_SECRET || !periodId || !token) return false;
  return safeCompare(token, signHolidayBoardToken(periodId));
}
