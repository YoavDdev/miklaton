...
export function buildReportHtml(snapshot, reportDateLabel) {
  ...
  // סמל העירייה בראש הדוח (בקשת יואב 26.08). origin מפורש - החלון
  // החדש נפתח על about:blank ונתיב יחסי לא היה נפתר.
  const origin = typeof window !== 'undefined' ? window.location.origin : '';
  const head = `<div class="report-head"><img src="${origin}/city-logo.png" alt="סמל העירייה"><div>עיריית יהוד-מונוסון</div></div>`;
...
<style>
  * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .report-head { display: flex; flex-direction: column; align-items: center; gap: 2px; margin-bottom: 12px; }
  .report-head img { height: 84px; }
  .report-head div { font-size: 15px; font-weight: bold; color: #1f2a63; }
...
export function openPrintPdf(snapshot, reportDateLabel) {
  ...
  win.document.close();
  win.focus();
  // print אחרי שהדפדפן סיים לפרסס ולטעון את הסמל - התמונה בדיאלוג תהיה שלמה
