...
export default function OperatorPage() {
  ...
  // התפקיד נדרש כדי להסתיר יכולות שאינן של נציג. קודם נשלף כאן isAdmin
  // ומעולם לא נקרא - ענף שנשאר חצי בנוי.
  const [role, setRole] = useState('');
  const canManageShift = ['shift_supervisor', 'call_center_manager', 'admin'].includes(role);
      ...
      <header className="bg-gradient-to-l from-slate-800 to-slate-900 text-white shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* נציג רואה את המצב אך אינו משנה אותו. ההחלטה היא של מי שמנהל
                את המשמרת - אחמ״ש או מנהלת המוקד. */}
            {canManageShift ? (
              <button
              ...
              <span
                title="רק אחמ״ש או מנהלת המוקד משנים את מצב החירום"
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-bold border-2 ${
                  warMode
